...
> 🧭 **設計・アーキテクチャ上の判断は作者によるもの**（MVP のレイヤー分割、フォルダ＝名前空間の命名方針、「Overdue」を保存する状態から `due_date` による導出へ変えた設計変更、Auto Save に至る経緯 など。根拠は「設計」セクション）。実装には AI（Claude Code）をペアプログラミング相手として併用しており、その旨をコミットの `Co-Authored-By` に残している。
