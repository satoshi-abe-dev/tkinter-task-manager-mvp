...
**15分おきに`app.db`の変更を検知して自動的にバックアップを取る**（前回のバックアップ以降に
変更が無ければスキップする）。`data/backups/`に保存され、直近**24時間**分だけ残して古いものは
...
その後、「保存前の誤操作をアプリ再起動だけで取り消せるようにしたい」という理由で即時書き込みをやめ、明示的な`save()`を待つ方式（Saveボタン・未保存表示・終了時の確認ダイアログ付き）に変更したこともあったが、最終的には「Saveボタンの置き場所に悩むくらいなら、そもそも自動保存にしてしまえばよいのでは」という結論に至り、常時Auto Save（今の形）に戻した。保存前の誤操作を取り消す手段はあきらめる代わりに、15分おきの自動バックアップ（下記参照）を「別の防御層」として残している。
