...
タブはあえてOS標準に近い見た目（`ttk.Notebook`のデフォルトスタイル）のままにしている。
...
| 層 | クラス | 責務 | 依存先 |
|---|---|---|---|
| Model | `TaskModel` | タスクの保持・追加のみ。UIのことは一切知らない。 | なし |
...
| Presenter | `TaskListPresenter` / `NewTaskPresenter` / `SettingsPresenter` | 各タブの「画面の振る舞い」のロジック。バリデーション・Model更新・タブ間の連携（新規登録後に一覧を更新するなど）を担う。 | 対応するModel, 対応するView（抽象のみ） |
