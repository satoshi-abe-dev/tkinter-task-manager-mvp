...
| 並べ替える | 列見出しをクリック。もう一度クリックで昇順⇄降順（見出しに ▲/▼）。優先度・ステータスは五十音順ではなく意味順（Low→High、Not Started→…→Overdue）。空欄の行は常に末尾 |
...
- 期限を**過去の日付**にインライン編集すると、その瞬間だけステータスが自動で「Overdue」になる（Done は対象外）。以降は手動で変えた値が優先される（常時は強制しない）。
- ステータスを手動で「Overdue」にした行は、期限が未来でも常に赤くなる（Done は対象外）。
- CSV インポート時は、ファイルに書かれたステータスをそのまま取り込み、上の自動セットは行わない。
...
> 💡 **動かすだけなら、ここまで読めば十分です。** この先は、このサンプルの主眼である「MVP で責務をどう分けたか」の解説です。

---

## 設計

このリポジトリの主眼。MVP（Model / View / Presenter）で責務をどう分けているかを解説する。
...
- **自動 Overdue は一度きり**: 期限を過去にした瞬間だけステータスを自動更新し、それ以降は強制しない。ユーザーの手動変更を尊重するため。
